...
static enum {
  APPEKG_STAT_OK=0, APPEKG_STAT_ERROR, APPEKG_STAT_DISABLED
...
static struct Metric {
   ...
   enum {UINT8, UINT16, UINT32, UINT64, INT8, INT16, INT32, 
         INT64, FLOAT, DOUBLE, STRING} type;
   union {
      uint8_t  u8val;
      uint16_t u16val;
      uint32_t u32val;
                  ...
                  unsigned int rank, unsigned int silent)
{
   ...
   _ekgHBCount = (unsigned int*) calloc(sizeof(unsigned int),
                                        EKG_MAX_HEARTBEATS * EKG_MAX_THREADS);
   _ekgActualThreadID = (unsigned int*) calloc(sizeof(unsigned int),
                                               EKG_MAX_THREADS);
...
void ekgEndHeartbeat(unsigned int hbId)
{
   ...
   { 
      ...
      avg = ((threadMetrics[thId][hbId + 1].v.dpval *
             (threadMetrics[thId][hbId].v.u64val) + duration) / 
...
static void outputLDMSStreamsData()
{
   ...
   for (i=0; i < metricCount; i++) {
      ...
      switch (metrics[i].type) {
       case UINT8: j += sprintf(&str[j], " \"%s\":%u, ", metrics[i].name, 
                                          metrics[i].v.u8val);
                   break;
       case UINT16: j += sprintf(&str[j],"\"%s\":%u", metrics[i].name,
                                         metrics[i].v.u16val);
                    break;
       case UINT32: j +=sprintf(&str[j],"\"%s\":%u", metrics[i].name,
                                        metrics[i].v.u32val); 
                    break;
       case UINT64: j += sprintf(&str[j],"\"%s\":%lu", metrics[i].name,
                                         metrics[i].v.u64val); 
                    break;
       case INT8: j += sprintf(&str[j],"\"%s\":%d", metrics[i].name,
                                         metrics[i].v.i8val); 
                    break;
       case INT16: j += sprintf(&str[j],"\"%s\":%d", metrics[i].name,
                                        metrics[i].v.i16val);
                    break;
       case INT32: j += sprintf(&str[j],"\"%s\":%d", metrics[i].name,
                                        metrics[i].v.i32val);
                   break;
       case INT64: j += sprintf(&str[j],"\"%s\":%ld", metrics[i].name,
                                         metrics[i].v.i64val);
                   break;
       case FLOAT: j += sprintf(&str[j],"\"%s\":%g", metrics[i].name,
                                         metrics[i].v.fpval);
                   break;
       case DOUBLE: j += sprintf(&str[j],"\"%s\":%g", metrics[i].name,
                                         metrics[i].v.dpval);
                   break;
       case STRING: j += sprintf(&str[j],"\"%s\":\"%s\"", metrics[i].name,
                                         metrics[i].v.strval);
                   break;
       default: j += sprintf(&str[j],"\"%s\":\"%s\"", metrics[i].name,"0"); 
                   break;
      }
   }
   j += sprintf(&str[j], "%s", "}");
...
static char* metadataToJSON()
{
   ...
   // i starts at 1 to skip timestamp
   for (i=1; i < baseCount; i++) {
      ...
      switch (baseMetrics[i].type) {
       case UINT8: 
        tstrlen += snprintf(tdatastr+tstrlen, sizeof(tdatastr)-tstrlen-1,
                            "\"%s\":%u",baseMetrics[i].name, 
                            baseMetrics[i].v.u8val); break;
       case UINT16: 
        tstrlen += snprintf(tdatastr+tstrlen, sizeof(tdatastr)-tstrlen-1,
                            "\"%s\":%u",baseMetrics[i].name, 
                            baseMetrics[i].v.u16val); break;
       case UINT32:
        tstrlen += snprintf(tdatastr+tstrlen, sizeof(tdatastr)-tstrlen-1,
                            "\"%s\":%u",baseMetrics[i].name, 
                            baseMetrics[i].v.u32val); break;
       case UINT64: 
        tstrlen += snprintf(tdatastr+tstrlen, sizeof(tdatastr)-tstrlen-1,
                            "\"%s\":%lu",baseMetrics[i].name, 
                            baseMetrics[i].v.u64val); break;
       case INT8: 
        tstrlen += snprintf(tdatastr+tstrlen, sizeof(tdatastr)-tstrlen-1,
                            "\"%s\":%d",baseMetrics[i].name, 
                            baseMetrics[i].v.i8val); break;
       case INT16: 
        tstrlen += snprintf(tdatastr+tstrlen, sizeof(tdatastr)-tstrlen-1,
                            "\"%s\":%d",baseMetrics[i].name, 
                            baseMetrics[i].v.i16val); break;
       case INT32: 
        tstrlen += snprintf(tdatastr+tstrlen, sizeof(tdatastr)-tstrlen-1,
                            "\"%s\":%d",baseMetrics[i].name, 
                            baseMetrics[i].v.i32val); break;
       case INT64: 
        tstrlen += snprintf(tdatastr+tstrlen, sizeof(tdatastr)-tstrlen-1,
                            "\"%s\":%ld",baseMetrics[i].name, 
                            baseMetrics[i].v.i64val); break;
       case FLOAT: 
        tstrlen += snprintf(tdatastr+tstrlen, sizeof(tdatastr)-tstrlen-1,
                            "\"%s\":%g",baseMetrics[i].name, 
                            baseMetrics[i].v.fpval); break;
       case DOUBLE: 
        tstrlen += snprintf(tdatastr+tstrlen, sizeof(tdatastr)-tstrlen-1,
                            "\"%s\":%g",baseMetrics[i].name, 
                            baseMetrics[i].v.dpval); break;
   ...
   tstrlen += snprintf(tdatastr+tstrlen, sizeof(tdatastr)-tstrlen-1,
                       ",\n\"starttime\":%u", (unsigned) curtime.tv_sec);
   ...
   // heartbeat names should be put in an array? Done.
   tstrlen += snprintf(tdatastr+tstrlen, sizeof(tdatastr)-tstrlen-1,
                       ",\n\"hbnames\":{");
   for (i=1; i <= numHeartbeats; i++) {
      if (hbNames[i] != 0) {
   ...
   for (tid=0; tid < EKG_MAX_THREADS; tid++) {
      ...
      for (i=0; i < baseCount; i++) {
         ...
         switch (baseMetrics[i].type) {
          case UINT8: 
            tstrlen += snprintf(tdatastr+tstrlen, sizeof(tdatastr)-tstrlen-1,
                               "%u",baseMetrics[i].v.u8val); break;
          case UINT16: 
            tstrlen += snprintf(tdatastr+tstrlen, sizeof(tdatastr)-tstrlen-1,
                               "%u",baseMetrics[i].v.u16val); break;
          case UINT32: 
            tstrlen += snprintf(tdatastr+tstrlen, sizeof(tdatastr)-tstrlen-1,
                               "%u",baseMetrics[i].v.u32val); break;
          case UINT64: 
            tstrlen += snprintf(tdatastr+tstrlen, sizeof(tdatastr)-tstrlen-1,
                               "%lu",baseMetrics[i].v.u64val); break;
          case INT8: 
            tstrlen += snprintf(tdatastr+tstrlen, sizeof(tdatastr)-tstrlen-1,
                               "%d",baseMetrics[i].v.i8val); break;
          case INT16: 
            tstrlen += snprintf(tdatastr+tstrlen, sizeof(tdatastr)-tstrlen-1,
                               "%d",baseMetrics[i].v.i16val); break;
          case INT32: 
            tstrlen += snprintf(tdatastr+tstrlen, sizeof(tdatastr)-tstrlen-1,
                               "%d",baseMetrics[i].v.i32val); break;
          case INT64: 
            tstrlen += snprintf(tdatastr+tstrlen, sizeof(tdatastr)-tstrlen-1,
                               "%ld",baseMetrics[i].v.i64val); break;
          case FLOAT: 
            tstrlen += snprintf(tdatastr+tstrlen, sizeof(tdatastr)-tstrlen-1,
                               "%g",baseMetrics[i].v.fpval); break;
          case DOUBLE: 
            tstrlen += snprintf(tdatastr+tstrlen, sizeof(tdatastr)-tstrlen-1,
                               "%g",baseMetrics[i].v.dpval); break;
          //case STRING: fprintf(csvFH,"%s",baseMetrics[i].v.strval); break;
          default: 
            tstrlen += snprintf(tdatastr+tstrlen, sizeof(tdatastr)-tstrlen-1,
                               "unknown"); break;
      ...
      for (i=1; i < metricCount; i++) {
         ...
         switch (threadMetrics[0][i].type) {
          case UINT8: 
            tstrlen += snprintf(tdatastr+tstrlen, sizeof(tdatastr)-tstrlen-1,
                               "%u",threadMetrics[tid][i].v.u8val);
            break;
          case UINT16:
            tstrlen += snprintf(tdatastr+tstrlen, sizeof(tdatastr)-tstrlen-1,
                               "%u",threadMetrics[tid][i].v.u16val); break;
          case UINT32:
            tstrlen += snprintf(tdatastr+tstrlen, sizeof(tdatastr)-tstrlen-1,
                               "%u",threadMetrics[tid][i].v.u32val); break;
          case UINT64:
            tstrlen += snprintf(tdatastr+tstrlen, sizeof(tdatastr)-tstrlen-1,
                               "%lu",threadMetrics[tid][i].v.u64val); break;
          case INT8:
            tstrlen += snprintf(tdatastr+tstrlen, sizeof(tdatastr)-tstrlen-1,
                               "%d",threadMetrics[tid][i].v.i8val); break;
          case INT16:
            tstrlen += snprintf(tdatastr+tstrlen, sizeof(tdatastr)-tstrlen-1,
                               "%d",threadMetrics[tid][i].v.i16val); break;
          case INT32:
            tstrlen += snprintf(tdatastr+tstrlen, sizeof(tdatastr)-tstrlen-1,
                               "%d",threadMetrics[tid][i].v.i32val); break;
          case INT64:
            tstrlen += snprintf(tdatastr+tstrlen, sizeof(tdatastr)-tstrlen-1,
                               "%ld",threadMetrics[tid][i].v.i64val); break;
          case FLOAT:
            tstrlen += snprintf(tdatastr+tstrlen, sizeof(tdatastr)-tstrlen-1,
                               "%.3f",(double) threadMetrics[tid][i].v.fpval);
            break;
          case DOUBLE:
            tstrlen += snprintf(tdatastr+tstrlen, sizeof(tdatastr)-tstrlen-1,
                               "%.3f",threadMetrics[tid][i].v.dpval); break;
...
static void outputHeartbeatData()
{
   ...
   baseMetrics[0].v.i64val = (curtime.tv_sec*1000) + 
